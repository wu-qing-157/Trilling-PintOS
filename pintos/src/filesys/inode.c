...
struct inode_disk
  {
    ...
    /* GXY's code begin */
    size_t allocated;
    uint32_t unused[(BLOCK_SECTOR_SIZE - (DIRECT_CNT + INDIRECT_CNT + DOUBLY_CNT) * sizeof(block_sector_t) - sizeof(off_t) - sizeof(unsigned) - sizeof(size_t) - sizeof(bool)) / sizeof(uint32_t)];
    /* GXY's code end */
  };
